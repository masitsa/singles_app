...
$$(".popup-splash").on("opened", function() {
    ...
    $(".zoom").swipebox(), $(".twitter-content").length > 0 && $(".twitter-content").twittie({
        count: 10
    ...
    /*(null === localStorage.getItem("newOptions") || localStorage.getItem("newOptions") === !0) && (myApp.popup(".popup-splash"), 
    localStorage.setItem("newOptions", !0));*/
				...
				if(register_data.message == "success")
				{
					...
					result_login.done(function (employees) 
					{
						...
						if(data.message == "success")
						{
							...
							$("#profile_username").html(window.localStorage.getItem("client_username")).fadeIn("slow");
							myApp.closeModal(".popup-login");
							myApp.closeModal(".register_modal");
							$( "#loader-wrapper" ).addClass( "display_none" );
							initialize_app();
						}
...
$(document).on("submit","form#forgot-password",function(e)
{
	...
	result.done(function (ret_login) {
		...
		try {
			...
			{
				myApp.closeModal(".popup-login");
				myApp.closeModal(".forgot_password_modal");
...
function load_single_message(web_name)
{
	...
	if(isNaN(prev_message_count))
	{
		...
		request.done(function (return_messages) {
		
			var data_messages = jQuery.parseJSON((return_messages);alert(data_messages.result);
			
			window.localStorage.setItem("message_history"+web_name, data_messages.result);
			window.localStorage.setItem("username"+web_name, data_messages.username);
			window.localStorage.setItem("receiver_id"+web_name, data_messages.receiver_id);
			window.localStorage.setItem("prev_message_count"+web_name, data_messages.received_messages);
		...
		alert('msg_2 = '+window.localStorage.getItem("message_history"+web_name));
	...
	else
	{
		...
		$("#ajax_receiver").val(window.localStorage.getItem("receiver_id"+web_name));
		$(".single_message").html(window.localStorage.getItem("message_history"+web_name));
		alert('msg_1 = '+window.localStorage.getItem("message_history"+web_name));
		$('#web_name').val(web_name);
		$('#message_receiver').html(web_name);
...
{
	var page = e.detail.page;
	var page_query = page.query;
	var web_name = page_query.web_name;
	
	if(web_name != null)
	{
		load_single_message(web_name);
	}
})